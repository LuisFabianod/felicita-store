...
exports.userRegister = async (req, res, next) => {
  ...
  const verificationCode = Math.floor(100000 + Math.random() * 900000).toString(); // criação de um código aleatório para verificação de e-mail
    ...
    await sendEmail(email, verificationCode);  // envia o código para o e-mail do usuário

    return res.status(200).json({ message: `Você está a um passo de criar sua conta! Enviamos um código para ${email}`}); // resposta avisando sobre o e-mail
  } 
